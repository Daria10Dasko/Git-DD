...
Мы должны зарегестрироваться на GitHub. Ввести необходимые данные следуя инструкции на сайте. Указать свою почту и создать пароль. Затем авторизоваться в Git, чтобы в дальнейшем данные приходили именно на ваш аккаунт.

### 14.2 Создать локальный репозиторий

Если у вас не создан локальный репозиторий, то следуйте слудеющим командам: 

```
echo "# JKNCKJD" >> README.md 
git init
git add README.md 
git commit -m "первая фиксация"
```

### 14.3 Cоздать удаленный репозиторий

Переходим на ваш аккуант GitHub и ищем ***CreateNew*** И создаем **New repository**. Вводим ваше название репозитория, а после этого строго следуем инструкции


### 14.4 Связать локальный репозиторий с удалёнными

Связываем локальный репозиторий и удаленный с помощью команды 

```
git remote add origin https://github.com/имя репозитория
git push -u origin main
```

## 15. Инструкция по работе с удалёнными репозиториями, pull request. 

Отображение удалённых репозиториев Чтобы просмотреть, какие удалённые серверы у вас уже настроены, следует выполнить команду 
``` 
git remote 
``` 
...
***Теперь вы можете использовать в командной строке имя pb вместо полного URL. Например, если вы хотите извлечь (fetch) всю информацию, которая есть в репозитории, но нет в вашем, вы можете выполнить git fetch pb***
...
Если хотите получить побольше информации об одном из удалённых репозиториев, вы можете использовать команду git remote show [удал. сервер]. Если вы выполните эту команду с некоторым именем, например, origin
